...
def rule_connectivity_checks(lport=None):
    ...
    if lport is not None:
        ...
        rules = [rule for rule in rules if int(rule["lport"]) == int(lport)]
        if not rules:
...
function checkRowProbe(btn,kind){let row=btn.closest('tr');if(kind==='target'){let ip=row?.querySelector('[data-label="IP"]')?.textContent.trim();if(ip)checkTargetLatency(btn,false,ip);return}let lport=row?.querySelector('[data-label="入口"]')?.textContent.trim()||btn.closest('.rule-card')?.querySelector('.route')?.textContent.split('→')[0].trim();if(lport)checkRuleConnectivity(btn,false,lport)}
...
async function checkRuleConnectivity(btn,quiet=false,lport=''){await runAction(btn,async()=>{let result=await api('/api/rules/connectivity',{method:'POST',body:JSON.stringify(lport?{lport}:{}),timeout:60000}),results=result.results||{};state.ruleConnectivity=lport?{...state.ruleConnectivity,...results}:results;render();if(!quiet)toast(lport?`已检测转发端口 ${lport} 的连通性`:'连通性检查完成','success')})}
