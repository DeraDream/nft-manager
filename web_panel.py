...
window.addEventListener('error',e=>{if(root&&!root.innerHTML)root.innerHTML=`<div class=login><form><h2>nft-manager</h2><p class=muted>前端加载失败</p><p>${e.message}</p><button type=button onclick="location.reload()" class=primary style="width:100%">刷新</button></form></div>`});
...
function shell(content){let n=[['dash','仪表板'],['rules','转发管理'],['targets','主机管理'],['settings','系统设置']].map(x=>`<button class="${state.view==x[0]?'active':''}" onclick="nav('${x[0]}')">${x[1]}</button>`).join('');root.innerHTML=`<div class=app><aside class=side><div class=brand>nft-manager</div><div class=ver>v1.2</div><div class=nav>${n}</div><div class=foot>Powered by nft-manager</div></aside><main class=main><div class=top><span class=user>admin ▾</span></div><div class=content>${content}</div></main></div>`}
...
async function saveTarget(old){await api('/api/targets/save',{method:'POST',body:JSON.stringify({oldIp:old?.ip,alias:ta.value,ip:tip.value})});document.querySelector('.modal').remove();await load()}
...
async function chgPwd(){await api('/api/password',{method:'POST',body:JSON.stringify({oldPassword:oldp.value,newPassword:newp.value})});alert('密码已修改');document.querySelector('.modal').remove()}
