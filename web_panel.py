...
function bandwidthChart(){let b=state.data.bandwidth||{},h=b.history||[],mobile=window.matchMedia('(max-width:760px)').matches,w=mobile?390:700,ht=mobile?250:320,left=72,right=mobile?10:18,top=24,bottom=mobile?42:44,pw=w-left-right,ph=ht-top-bottom,max=niceBandwidthMax(Math.max(0,...h.flatMap(x=>[Number(x.downloadMbps||0),Number(x.uploadMbps||0)]))),den=Math.max(1,h.length-1),x=i=>left+i*pw/den,y=value=>top+(1-Number(value||0)/max)*ph,download=h.map((item,i)=>`${x(i)},${y(item.downloadMbps)}`),upload=h.map((item,i)=>`${x(i)},${y(item.uploadMbps)}`),yGrid=[0,1,2,3,4].map(i=>{let yy=top+i*ph/4,value=max*(4-i)/4;return `<line class=chart-grid x1="${left}" y1="${yy}" x2="${w-right}" y2="${yy}"/><text class=chart-label text-anchor=end x="${left-7}" y="${yy+4}">${fmtBandwidth(value)}</text>`}).join(''),step=Math.max(1,Math.ceil(h.length/7)),xGrid=h.map((item,i)=>(i%step===0||i===h.length-1)?`<line class=chart-grid x1="${x(i)}" y1="${top}" x2="${x(i)}" y2="${top+ph}"/><text class=chart-label text-anchor=middle x="${x(i)}" y="${top+ph+15}">${bandwidthTime(item.timestamp)}</text>`:'').join(''),svg=`<svg class=chart-svg viewBox="0 0 ${w} ${ht}" data-left="${left}" data-width="${pw}" data-top="${top}" data-height="${ph}" data-max="${max}" onpointermove="chartHover(event,'bandwidth')" onpointerleave="hideChartTooltip(event)" role="img" aria-label="最近24小时上传下载带宽折线图"><g>${yGrid}${xGrid}</g><line class=chart-axis x1="${left}" y1="${top}" x2="${left}" y2="${top+ph}"/><line class=chart-axis x1="${left}" y1="${top+ph}" x2="${w-right}" y2="${top+ph}"/>${download.length?`<polyline class="chart-line bandwidth-download" points="${download.join(' ')}"/><polyline class="chart-line bandwidth-upload" points="${upload.join(' ')}"/>`:''}<line class=chart-hover-guide y1="${top}" y2="${top+ph}"/><circle class="chart-hover-dot download" r=5/><circle class="chart-hover-dot upload" r=5/><text class=chart-axis-title x="${left}" y="13">带宽</text><text class=chart-axis-title text-anchor=middle x="${left+pw/2}" y="${ht-3}">时间（24小时）</text></svg>`;return chartWrap(svg)}
