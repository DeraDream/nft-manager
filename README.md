...
纯 SSH 菜单版本升级到 Web 版本时，请在旧菜单中选择 `2) 更新脚本`。更新完成后重新执行一次：

```bash
sudo nft
```

新版会识别到旧管理器尚未部署 Web 面板，仅补装 Web 服务和 NextTrace，不会执行菜单 `1)` 的清空安装流程。Web 服务启动时会先校验旧转发配置，校验通过后才迁移为支持流量统计的格式；若校验或加载失败，会保留并恢复旧配置。
