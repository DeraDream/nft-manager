...
async function probeAndSort(kind,key,btn){if(kind==='target'&&key==='latency')await checkTargetLatency(btn);else if(kind==='rule'&&key==='connectivity')await checkRuleConnectivity(btn);setListSort(kind,key)}
function sortButton(kind,key,label){let sort=listSort(kind),active=sort.key===key,arrow=active?(sort.direction==='asc'?'↑':'↓'):'↕';return `<button class="sort-head ${active?'active':''}" onclick="probeAndSort('${kind}','${key}',this)" title="按${label}${active&&sort.direction==='asc'?'降序':'升序'}排列">${label}<span class=sort-arrow>${arrow}</span></button>`}
