...
RULE_RATE_STATE = {"timestamp": 0.0, "counters": {}, "activity": {}, "rates": {}, "available": False}
BANDWIDTH_RETENTION = 24 * 60 * 60
BANDWIDTH_SAMPLE_INTERVAL = 10
RULE_COUNTER_SAMPLE_INTERVAL = 10
RULE_ACTIVITY_WINDOW_SECONDS = 20
RULE_RATE_MIN_INTERVAL = 0.8
...
def rule_connectivity_checks(lport=None):
    checks = {}
    rules = parse_rules()
    if lport is not None:
        if isinstance(lport, bool) or not str(lport).isdigit():
            raise ValueError("转发端口无效")
        lport = int(lport)
        if not valid_port(lport):
            raise ValueError("转发端口无效")
        rules = [rule for rule in rules if int(rule["lport"]) == lport]
        if not rules:
            raise ValueError("转发规则不存在")
    for rule in rules:
...
def set_account(old_password, new_username, new_password=""):
    user, salt_b64, digest_b64 = read_auth()
    ...
    if not new_username or len(new_username) > 32:
        raise ValueError("用户名长度应为 1-32 个字符")
    if "|" in new_username or any(ch.isspace() or unicodedata.category(ch).startswith("C") for ch in new_username):
        raise ValueError("用户名不能包含空格或特殊控制字符")
    if new_password:
        if len(new_password) < 4:
            raise ValueError("新密码至少 4 位")
        salt_b64, digest_b64 = password_hash(new_password)
    if new_username == user and not new_password:
        raise ValueError("用户名和密码均未修改")
    atomic_write(AUTH_FILE, f"{new_username}|{salt_b64}|{digest_b64}\n")
    os.chmod(AUTH_FILE, 0o600)


def set_password(old_password, new_password):
    user, _, _ = read_auth()
    set_account(old_password, user, new_password)
...
def persisted_rule_activity():
    ...
    try:
        ...
        return {}
    activity = saved.get("activity", {}) if isinstance(saved, dict) and saved.get("version") == 2 else {}
    return {key: nonnegative_int(value) for key, value in activity.items()} if isinstance(activity, dict) else {}


def rule_rate_snapshot():
    with RULE_RATE_LOCK:
        now = time.time()
        previous = RULE_RATE_STATE
        elapsed = now - float(previous.get("timestamp", 0) or 0)
        if previous.get("timestamp") and elapsed < RULE_RATE_MIN_INTERVAL:
            return {
        ...
        available, current = read_kernel_counters_with_status()
        if not available:
            RULE_RATE_STATE["available"] = False
            return {"available": False, "sampledAt": now, "rates": previous.get("rates", {})}

        old_counters = previous.get("counters", {})
        activity = dict(previous.get("activity", {}))
        if not previous.get("timestamp"):
            activity.update(persisted_rule_activity())
        valid_delta = bool(previous.get("timestamp")) and 0.5 <= elapsed <= 5
        rates = {}

        for key, value in current.items():
            old = normalize_counter(old_counters.get(key, {}))
            current_counter = normalize_counter(value)
            direction_rates = {}
            traffic_delta = 0
            for direction in ("upload", "download"):
                current_bytes = current_counter[direction]["bytes"]
                old_bytes = old[direction]["bytes"]
                delta = current_bytes - old_bytes if current_bytes >= old_bytes else current_bytes
                delta = max(0, delta) if valid_delta else 0
                traffic_delta += delta
                direction_rates[f"{direction}Bps"] = round(delta / elapsed, 1) if valid_delta else 0.0
            if traffic_delta > 0:
                activity[key] = int(now)
            direction_rates["active"] = now - int(activity.get(key, 0)) <= RULE_ACTIVITY_WINDOW_SECONDS
            rates[key] = direction_rates

        RULE_RATE_STATE.update(
            {"timestamp": now, "counters": current, "activity": activity, "rates": rates, "available": True}
        )
        return {"available": True, "sampledAt": now, "rates": rates}
...
def nft_counters():
    with STATS_LOCK, stats_file_lock():
        ...
        for key, val in current.items():
            ...
            result[key] = {
                **total,
                "active": now - int(activity.get(key, 0)) <= RULE_ACTIVITY_WINDOW_SECONDS,
                "sampled_at": now,
            }
...
.rule-groups{gap:12px}.rule-group-toggle{min-height:58px;padding:9px 12px}.rule-group-title{display:grid;gap:1px}.rule-group-title strong{font-size:14px}.rule-group-ip{font-size:12px}.rule-group-count{font-size:11px}.rule-group .rule-grid{padding:12px}.rule-group .table tbody{padding:12px}.rule-group .table tr{box-shadow:none}
}
@media(max-width:760px){.top{z-index:9}.user-menu summary{font-size:12px;color:var(--muted)}.user-menu-popover{position:fixed;top:47px;right:10px;width:min(210px,calc(100vw - 20px))}}
...
function shell(content){let n=[['dash','仪表板','⌂'],['rules','转发管理','⇄'],['targets','主机管理','▣'],['firewall','防火墙','◫'],['settings','设置','⚙']].map(x=>`<button class="${state.view==x[0]?'active':''}" onclick="nav('${x[0]}')"><span class=nav-icon aria-hidden=true>${x[2]}</span><span>${x[1]}</span></button>`).join(''),title=esc(state.data.settings?.panelTitle||'nft-manager'),username=esc(state.data.username||'admin');appRoot.innerHTML=`<div class=app><aside class=side><div class=brand>${title}</div><div class=ver>v__WEB_PANEL_VERSION__</div><div class=nav>${n}</div><div class=foot>Powered by nft-manager</div></aside><main class=main><div class=top><span class=mobile-brand>${title}</span><details class=user-menu><summary>${username} ▾</summary><div class=user-menu-popover><button type=button onclick="openAccount()"><span aria-hidden=true>✎</span>修改用户名和密码</button><button type=button class=logout onclick="logout()"><span aria-hidden=true>↪</span>退出登录</button></div></details></div><div class=content>${content}</div></main></div>`}
...
function render(){let d=state.data;if(state.view==='dash'){let b=d.bandwidth||{};return shell(`<div class=cards><div class=card><h3>总流量</h3><div class=big>无限制</div><div class=bar></div></div><div class=card><h3>已用流量</h3><div class=big>${fmt(d.stats.totalBytes)}</div><div class=bar></div></div><div class=card><h3>转发配额</h3><div class=big>无限制</div><div class=bar></div></div><div class=card><h3>已用转发</h3><div class=big>${d.stats.ruleCount}</div><div class=bar></div></div></div><div class=chart-pair><div class=panel><h2>24小时流量统计</h2>${trafficChart()}</div><div class=panel><div class=chart-panel-head><h2>24小时带宽统计</h2><div class=bandwidth-live><span class=download id=bandwidthLiveDownload>下载 ${fmtBandwidth(b.downloadMbps)}</span><span class=upload id=bandwidthLiveUpload>上传 ${fmtBandwidth(b.uploadMbps)}</span><span class=bandwidth-interface id=bandwidthLiveInterface>${b.available?esc(b.interface):'未检测到出口网卡'}</span></div></div><div class=chart-legend><span class=download><i></i>下载</span><span class=upload><i></i>上传</span></div><div id=bandwidthLiveChart>${bandwidthChart()}</div></div></div><div class=panel><h2>转发配置 <span class=muted>${d.stats.ruleCount}</span></h2>${rulesTable(true)}</div>`)}
 if(state.view==='rules'){let next=state.ruleView==='flat'?'grid':'flat',label=state.ruleView==='flat'?'平铺':'方块',icon=state.ruleView==='flat'?'☷':'▦',allExpanded=ruleGroupsAllExpanded(),expandLabel=allExpanded?'一键收起':'一键展开',expandIcon=allExpanded?'▾':'▸';return shell(`<div class=page-with-fabs>${fabStack([fabButton(expandIcon,expandLabel,`toggleAllRuleGroups()`,false,expandLabel+'全部主机分区'),fabButton(icon,label,`setRuleView('${next}')`,false,`切换为${next==='grid'?'方块':'平铺'}视图`),fabButton('⌁','连通性',`checkRuleConnectivity(this)`,false,'检查全部转发连通性'),fabButton('+','新增转发',`openRule()`,true),fabButton('↺','默认排序',`resetListSort('rule')`,false,'清除转发列表排序')])}${rulesTable(false)}</div>`)}
 if(state.view==='targets')return shell(`<div class=page-with-fabs>${fabStack([fabButton('◷','延迟检测',`checkTargetLatency(this)`),fabButton('+','新增主机',`openTarget()`,true),fabButton('↺','默认排序',`resetListSort('target')`,false,'清除主机列表排序')])}<div class=panel><h2>主机管理</h2>${targetsTable()}</div></div>`);
 if(state.view==='firewall')return shell(`<div class=page-with-fabs>${fabStack([fabButton('↻','同步端口',`syncFirewall(this)`),fabButton('+','开放端口',`openFirewall()`,true)])}<div class=panel><h2>防火墙管理</h2><p class=muted>默认拒绝未列出的入站连接，始终保留当前 SSH 端口与 Web 面板 5555/tcp。</p>${firewallTable()}</div></div>`);
 return shell(`<div class=panel><h2>系统设置</h2><div class=field style="max-width:520px"><label>顶部标题</label><div class=actions><input id=panelTitle maxlength=64 value="${esc(d.settings?.panelTitle||'nft-manager')}"><button class=primary onclick="savePanelTitle(this)">保存标题</button></div></div><div class=field style="max-width:520px"><label>首页轮询间隔（秒）</label><div class=actions><input id=dashboardPollSeconds type=number min=2 max=300 step=1 value="${Number(d.settings?.dashboardPollSeconds||10)}"><button class=primary onclick="savePollInterval(this)">保存间隔</button></div><p class=muted>控制流量、规则等完整首页数据刷新，范围 2-300 秒；带宽速率在首页可见时固定每秒刷新。</p></div><div class=field><label>显示模式</label><div class=theme-switch><button class="${state.theme==='light'?'active':''}" onclick="setTheme('light')">日间</button><button class="${state.theme==='dark'?'active':''}" onclick="setTheme('dark')">夜间</button><button class="${state.theme==='system'?'active':''}" onclick="setTheme('system')">跟随系统</button></div></div><section class=update-section><div class=update-section-head><div><h3>在线更新</h3><p class=muted>当前版本 <span class=update-version>v__WEB_PANEL_VERSION__</span>，升级期间 Web 服务会短暂重启。</p></div><button class=primary onclick="checkOnlineUpdate(this)">检查更新</button></div></section><div class=config-transfer><h3>配置迁移</h3><p class=muted>导出全部主机与端口转发；导入时追加到现有配置，并自动同步转发端口防火墙。</p><div class=config-actions><button onclick="exportConfig(this)">导出配置</button><button class=primary onclick="openConfigImport()">导入配置</button></div></div><p>Web 面板地址：<span class=pill>${esc(location.origin)}</span></p><p>默认端口：5555</p><button onclick="openAccount()">修改用户名和密码</button> <button class=danger onclick="showInfo('完整卸载','请在 SSH 菜单执行完整卸载')">完整卸载</button></div>`)}
function setRuleView(view){state.ruleView=view;localStorage.setItem('nft_manager_rule_view',view);render()}
function ruleGroupIps(){return [...new Set([...(state.data?.targets||[]).map(t=>t.ip),...(state.data?.rules||[]).map(r=>r.ip)])]}
function saveCollapsedRuleGroups(){localStorage.setItem('nft_manager_collapsed_rule_groups',JSON.stringify([...state.collapsedRuleGroups]))}
function ruleGroupsAllExpanded(){let ips=ruleGroupIps();return !ips.some(ip=>state.collapsedRuleGroups.has(ip))}
function toggleRuleGroup(ip){if(state.collapsedRuleGroups.has(ip))state.collapsedRuleGroups.delete(ip);else state.collapsedRuleGroups.add(ip);saveCollapsedRuleGroups();render()}
function toggleAllRuleGroups(){let ips=ruleGroupIps();if(ruleGroupsAllExpanded())ips.forEach(ip=>state.collapsedRuleGroups.add(ip));else ips.forEach(ip=>state.collapsedRuleGroups.delete(ip));saveCollapsedRuleGroups();render()}
...
function groupedRules(rules){let byIp=new Map();(state.data.targets||[]).forEach(t=>byIp.set(t.ip,{ip:t.ip,alias:t.alias||t.ip,rules:[]}));rules.forEach(r=>{if(!byIp.has(r.ip))byIp.set(r.ip,{ip:r.ip,alias:r.targetAlias||r.ip,rules:[]});byIp.get(r.ip).rules.push(r)});return [...byIp.values()]}
function ruleGroupSection(group,content){let collapsed=state.collapsedRuleGroups.has(group.ip),count=`${group.rules.length} 条转发`;return `<section class="rule-group ${collapsed?'collapsed':''}"><button type=button class=rule-group-toggle onclick='toggleRuleGroup(${JSON.stringify(group.ip)})' aria-expanded="${!collapsed}" title="${collapsed?'展开':'收起'} ${esc(group.alias)}"><span class=rule-group-chevron aria-hidden=true>${collapsed?'▸':'▾'}</span><span class=rule-group-title><strong>${esc(group.alias)}</strong><span class=rule-group-ip>${esc(group.ip)}</span></span><span class=rule-group-count>${count}</span></button>${collapsed?'':`<div class=rule-group-body>${content}</div>`}</section>`}
function ruleCardMarkup(r,showTarget=true){let active=ruleIsActive(r);return `<article class="rule-card ${r.enabled?'':'disabled'}"><div class=rule-card-top><h3>${esc(ruleName(r))}</h3>${ruleSwitch(r)}</div>${showTarget?`<div class=muted>${esc(r.targetAlias||'-')} / ${esc(r.ip)}</div>`:''}<div class=route>${r.lport} → ${r.dport}</div><div class=metrics><span class="metric upload">上传<br>${fmt(r.uploadBytes)}</span><span class="metric download">下载<br>${fmt(r.downloadBytes)}</span><span class="metric total">总计<br>${fmt(r.totalBytes)}</span></div>${rulePolicySummary(r)}<p data-rule-status="${r.lport}">${policyBadge(r,active)}</p><div class="live-rate-slot mobile-rate-card" data-rule-rate="${r.lport}">${liveRateMarkup(r,active)}</div>${probePill(state.ruleConnectivity[r.lport],'连通性')}<div class=actions>${ruleActions(r)}</div><div class="live-rate-slot desktop-rate-card" data-rule-rate="${r.lport}">${liveRateMarkup(r,active)}</div></article>`}
function ruleRowMarkup(r,showTarget=true){let active=ruleIsActive(r);return `<tr>${showTarget?`<td data-label="目标主机">${esc(r.targetAlias||'-')}<br><span class=muted>${esc(r.ip)}</span></td>`:''}<td data-label="别名">${esc(ruleName(r))}</td><td data-label="入口">${r.lport}</td><td data-label="出口">${r.dport}</td><td data-label="上传" class="metric upload">${fmt(r.uploadBytes)}</td><td data-label="下载" class="metric download">${fmt(r.downloadBytes)}</td><td data-label="总计" class="metric total">${fmt(r.totalBytes)}</td><td data-label="实时" class="live-rate-slot mobile-rate-cell" data-rule-rate="${r.lport}">${liveRateMarkup(r,active)}</td><td data-label="策略">${rulePolicySummary(r)}</td><td data-label="连通性">${probePill(state.ruleConnectivity[r.lport],'连通性')}</td><td data-label="状态" data-rule-status="${r.lport}">${policyBadge(r,active)}</td><td data-label="操作" class=actions>${ruleSwitch(r)}${ruleActions(r)}</td><td data-label="实时" class="live-rate-slot desktop-rate-cell" data-rule-rate="${r.lport}">${liveRateMarkup(r,active)}</td></tr>`}
function rulesTable(limit,sourceRules=null){
 ...
 if(sortable){let groups=groupedRules(rules);if(!groups.length)return `${controls}<div class=rule-table-wrap><table class=table><tbody><tr><td class=muted>暂无转发</td></tr></tbody></table></div>`;if(state.ruleView==='grid'){let sections=groups.map(group=>ruleGroupSection(group,`<div class=rule-grid>${group.rules.map(r=>ruleCardMarkup(r,false)).join('')||'<span class=muted>暂无转发</span>'}</div>`)).join('');return `${controls}<div class=rule-groups>${sections}</div>`}let upload=sortButton('rule','upload','上传'),download=sortButton('rule','download','下载'),total=sortButton('rule','total','总计'),connectivity=sortButton('rule','connectivity','连通性'),head=`<thead><tr><th>别名</th><th>入口</th><th>出口</th><th class="metric upload">${upload}</th><th class="metric download">${download}</th><th class="metric total">${total}</th><th>策略</th><th>${connectivity}</th><th>状态</th><th>操作</th><th>实时</th></tr></thead>`,sections=groups.map(group=>{let rows=group.rules.map(r=>ruleRowMarkup(r,false)).join('')||'<tr><td colspan=11 class=muted>该主机暂无转发</td></tr>';return ruleGroupSection(group,`<div class=rule-table-wrap><table class=table>${head}<tbody>${rows}</tbody></table></div>`)}).join('');return `${controls}<div class=rule-groups>${sections}</div>`}
 let rows=rules.map(r=>ruleRowMarkup(r,true)).join('');
...
async function checkTargetLatency(btn,quiet=false,ip=''){await runAction(btn,async()=>{let result=await api('/api/targets/latency',{method:'POST',body:JSON.stringify(ip?{ip}:{}),timeout:60000}),results=result.results||{};state.targetLatency=ip?{...state.targetLatency,...results}:results;render();if(!quiet)toast(ip?`已检测主机 ${ip} 的延迟`:'延迟检测完成','success')})}
async function checkRuleConnectivity(btn,quiet=false,lport=''){await runAction(btn,async()=>{let port=lport===''?'':Number(lport);if(port!==''&&(!Number.isInteger(port)||port<1||port>65535))throw new Error('转发端口无效');let result=await api('/api/rules/connectivity',{method:'POST',body:JSON.stringify(port!==''?{lport:port}:{}),timeout:60000}),results=result.results||{};state.ruleConnectivity=port!==''?{...state.ruleConnectivity,...results}:results;render();if(!quiet)toast(port!==''?`已检测转发端口 ${port} 的连通性`:'连通性检查完成','success')})}
...
async function saveAccount(btn){let layer=btn.closest('.modal'),oldPassword=layer.querySelector('#oldp').value,newUsername=layer.querySelector('#newu').value.trim(),newPassword=layer.querySelector('#newp').value,confirmPassword=layer.querySelector('#confirmp').value;if(newPassword!==confirmPassword){setModalError('两次输入的新密码不一致',layer);return}await runAction(btn,async()=>{await api('/api/account',{method:'POST',body:JSON.stringify({oldPassword,newUsername,newPassword})});layer.remove();expireSession();toast('账号已修改，请重新登录','success')})}
async function logout(){document.querySelector('.user-menu')?.removeAttribute('open');try{await api('/api/logout',{method:'POST',body:'{}',timeout:5000})}catch(e){}finally{expireSession();toast('已退出登录','success')}}
...
boot();
document.addEventListener('click',event=>{let menu=document.querySelector('.user-menu[open]');if(menu&&!menu.contains(event.target))menu.removeAttribute('open')});
document.addEventListener('visibilitychange',()=>{syncBandwidthLive();syncRuleRates();if(!document.hidden)pollDashboard()});
</script></body></html>"""
    ...
    def do_POST(self):
        try:
            ...
            if path == "/api/logout":
                self.send_json(
                    {"ok": True},
                    headers={"Set-Cookie": "sid=; Max-Age=0; HttpOnly; Path=/; SameSite=Lax"},
                )
                return
            if not self.require():
                return
            ...
            if path == "/api/password":
                ...
                self.send_json({"results": target_latency_checks(data.get("ip"))})
                return
