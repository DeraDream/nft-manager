...
function trafficChart(){let h=state.data.history||[],w=960,ht=300,left=76,right=22,top=18,bottom=50,pw=w-left-right,ph=ht-top-bottom,max=Math.max(1,...h.map(x=>x.bytes)),den=Math.max(1,h.length-1),x=i=>left+i*pw/den,y=b=>top+(1-b/max)*ph,pts=h.map((item,i)=>`${x(i)},${y(item.bytes)}`),area=pts.length?`${left},${top+ph} ${pts.join(' ')} ${x(h.length-1)},${top+ph}`:'',yGrid=[0,1,2,3,4].map(i=>{let yy=top+i*ph/4,value=max*(4-i)/4;return `<line class=chart-grid x1="${left}" y1="${yy}" x2="${w-right}" y2="${yy}"/><text class=chart-label text-anchor=end x="${left-10}" y="${yy+4}">${fmt(value)}</text>`}).join(''),xGrid=h.map((item,i)=>i%3===0?`<line class=chart-grid x1="${x(i)}" y1="${top}" x2="${x(i)}" y2="${top+ph}"/><text class=chart-label text-anchor=middle x="${x(i)}" y="${ht-26}">${hourText(item.hour)}</text>`:'').join('');return `<svg class=chart-svg viewBox="0 0 ${w} ${ht}" role="img" aria-label="最近24小时流量折线图"><g>${yGrid}${xGrid}</g><line class=chart-axis x1="${left}" y1="${top}" x2="${left}" y2="${top+ph}"/><line class=chart-axis x1="${left}" y1="${top+ph}" x2="${w-right}" y2="${top+ph}"/>${area?`<polygon class=chart-fill points="${area}"/><polyline class=chart-line points="${pts.join(' ')}"/>`:''}<text class=chart-axis-title text-anchor=middle x="${left+pw/2}" y="${ht-3}">时间（小时）</text><text class=chart-axis-title text-anchor=middle transform="rotate(-90 16 ${top+ph/2})" x="16" y="${top+ph/2}">流量</text></svg>`}
